...
const Addroom = () => {
  ...
  return (
    <form action="" >
      <Title align="left" font='outfit' title='Add Room' subTitle='Fill in the details carefullly and accurate room details,price and amenities to enhance the user booking experience.'/>
      ...
      <div className='flex flex-col flex-wrap mt-1 text-gray-400 max-w-sm'>
        {Object.keys(input.amenities).map((amenity, index) => (
          <div key={index}>
            ...
            checked={input.amenities[amenity]} onChange={() => setInput({...input, amenities: {...input.amenities, [amenity]: !input.amenities[amenity]}})} />
            <label htmlFor={`amenities${index+1}`}>{amenity}</label>
          </div>
        ))}
      </div>
      <button className='bg-primary text-white px-8 py-2 rounded mt-8 cursor-pointer mb-5'>Add Room</button>
    </form>
  )
}
